...
#define TPE_JOINT_SIZE(joint) ((joint).sizeDivided * TPE_JOINT_SIZE_MULTIPLIER)
  ...
  #define TPE_APPROXIMATE_LENGTH 1      /**< whether or not use length/distance 
...
TPE_Vec3 TPE_envAABoxInsideAA(TPE_Vec3 point, TPE_Vec3 aabbMin, TPE_Vec3 aabbMax)
{
  TPE_Vec3 shifted = TPE_vec3Minus(point, aabbMin);

  TPE_Vec3 a = TPE_vec3Minus(aabbMax, point),
           b = shifted;
  ...
  {
    if (a.x < a.z)
      point.x = (sx == 1) ? aabbMax.x : aabbMin.x;
    else
      point.z = (sz == 1) ? aabbMax.z : aabbMin.z;
  }
  else
  {
    if (a.y < a.z)
      point.y = (sy == 1) ? aabbMax.y : aabbMin.y;
    else
      point.z = (sz == 1) ? aabbMax.z : aabbMin.z;
  }
