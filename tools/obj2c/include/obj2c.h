...
{
    public:
        obj2c(const fs::path& path, const fs::path& output, const int verticeScale, const int textureSizeU, const int textureSizeV, const int normalScale, const std::string& type = "obj");
        void generate();

    private:
        void processLines(const fs::path& path);
        void processMDL(const fs::path& path);
