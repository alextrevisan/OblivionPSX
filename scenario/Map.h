...
extern uint32_t dead_tree128_tim[];
extern uint32_t grass01_tim[];
extern uint32_t grass_tile01_tim[];
    ...
    int Render(int mapPosX, int mapPosZ, MATRIX &mtx, const MATRIX& lightMTX)
    {
        ...
        for (int x = 0; x < MAP_SIZE - 1; ++x)
        {
            ...
            for (int z = 0; z < MAP_SIZE - 1; ++z)
            {
                ...
                Graphics->Draw<POLY_FT4, true>(vertices, normal, &grass_tile01_texture);
